...
const Dashboard = () => {
  ...
  // Reusable generic view modal state
  ...
  // Load notes on page change, search change, or itemsPerPage change
  useEffect(() => {
    if (user) {
  ...
  // Listen to automatic logout events from API client interceptor
  useEffect(() => {
  ...
  // Handle note creator click outside to collapse
  useEffect(() => {
  ...
  // Front-end real-time validation helper
  ...
  const handleTitleChange = (e) => {
    ...
    if (val.length <= 120) { // allow slightly more to trigger validation error visual
      setTitle(val);
      if (isExpanded) {
  ...
  const handleAddNote = (e) => {
    ...
    dispatch(addNote({
      ...
      if (addNote.fulfilled.match(action)) {
        ...
        // Refresh the current page notes list
        dispatch(fetchNotes({ page: currentPage, search: debouncedSearch, limit: itemsPerPage }));
      } else {
      ...
      onConfirm: () => {
        ...
        dispatch(deleteNote(noteId)).then((action) => {
          if (deleteNote.fulfilled.match(action)) {
            ...
            // Fetch again if the page becomes empty
